...
!(function($){	
    ...
    ScaleLazyApp.Core.Sandbox = Sandbox;
...
!(function($){
	...
	$.extend(Mediator.prototype, {
		...
		*/
		...
		/**
		* @method: unsubscribe，有多种方式可以移除监听事件
		* @param: ch: 要监听事件的名称（string|function）
		* @param: cb: 要移除监听的回调函数（可选）
		*/
		unsubscribe: function(ch, cb) {
		  var id;
		  switch (typeof ch) {
			case "string":
		...
		*/
		publish: function(channel, data, publishReference) {
	...
	*/
	...
	ScaleLazyApp.Core.Mediator = Mediator;
